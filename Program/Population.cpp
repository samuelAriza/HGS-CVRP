...
void Population::generatePopulation()
{
    ...
    for (int i = 0; i < 4*params.ap.mu && (i == 0 || params.ap.timeLimit == 0 || (double)(clock() - params.startTime) / (double)CLOCKS_PER_SEC < params.ap.timeLimit); i++)
    {
        ...
        if (i == 0) { // Educación de élite para el primer individuo
            rnr.apply(randomIndiv, std::floor(params.gamma * params.nbClients * 2));
        }
...
Population::Population(Params& params, Split& split, LocalSearch& localSearch, RuinAndRecreate& rnr) 
    : params(params), split(split), localSearch(localSearch), rnr(rnr), bestSolutionRestart(params), bestSolutionOverall(params)
{
