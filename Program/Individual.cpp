...
void Individual::removeCustomer(int customer) {
    ...
    chromT.erase(std::remove(chromT.begin(), chromT.end(), customer), chromT.end());
}

void Individual::insertCustomer(int customer, int route, int pos) {
    chromR[route].insert(chromR[route].begin() + pos, customer);
    chromT.clear();
    for (const auto& r : chromR) {
        chromT.insert(chromT.end(), r.begin(), r.end());
    }
}
